    ...
    def cross_violation(self, frame):
        ...
        for c in cnts:
            ...
            if self.light == "Red" and self.zone1[0] < (x + w / 2) < self.zone2[0] and self.zone1[1] > (y + h / 2) > \
                ...
                rcar = self.frame[max(0, y):max(0, y + h), max(0, x):max(0, x + w)]
                if rcar is not None and rcar.size > 0 and len(rcar.shape) >= 2 and rcar.shape[0] > 0 and rcar.shape[1] > 0:
                    rcar = cv2.resize(rcar, (0, 0), fx=4, fy=4)
                    cropped_cars.append(rcar)
                    # Save snapshot to disk only in GUI mode (not inside Spark headless workers)
                    import os
