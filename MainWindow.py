    ...
    def update_image(self):
        ...
        if len(cars_violated) > 0:
            for c in cars_violated:
                ...
                rule_id = '3' if self.cam_selector.currentText() == 'cam_04' else '1'
                self.database.insert_into_violations(camera=self.cam_selector.currentText(), car=carId, rule=rule_id,
                                                     time=time.time())
